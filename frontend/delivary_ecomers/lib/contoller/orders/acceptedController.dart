...
import 'package:delivary_ecomers/data/datasourse/remote/orders/accepted.dart';
...
class OrderAcceptedsController extends GetxController {
  ...
  doneOrder(String orderId,String usersId) async {
    ...
    var response = await orderAcceptedgData.doneOrder(
      orderId,usersId
    );
    ...
    if (statusRequest == StatusRequest.success) {
      if (response['status'] == 'success') {
        // List dataBody = response['data'];
        // data.addAll(dataBody.map((e) => OrderPendingModel.fromJson(e)));
        Get.snackbar("success","approve order");
        getData();
      } else {
        statusRequest = StatusRequest.failure;
      }
